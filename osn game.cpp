/*
    -For R=0, we can ask 1 2 3 ... n-1 n n n-1 ... 2 1. After that, until we get 2 hits, just ask 1 (or any other number). When we get 2 hits, we can uniquely determine what the bomb is (go through every k from 0 to n-1 and see if it works). So just print that number until we get n hits.
    -For R=1, we change our strategy a little bit: ask 1 2 3 ... n/2-1 n/2 n/2 n/2-1 ... 2 1.
        if(we get a hit){
            ask 1 until we have 2 hits, determine what the number is the same way we did for R=0, and ask it until we get n hits;
        }else{
