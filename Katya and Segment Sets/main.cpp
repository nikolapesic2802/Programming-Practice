/*
    -Make a segment tree where in every node you keep a set containing the pair {left border, the minimum right border of an interval that starts after the current one} for every interval corresponding to that node
    -We can construct this tree in n log^2 n time and answer every query online in log^2 n by using the lower_bound operation on segment trees.
*/
#include <bits/stdc++.h>
